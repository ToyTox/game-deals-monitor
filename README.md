...
| `npm test` | Прогнать тесты один раз (`vitest run`) |
| `npm run test:watch` | Тесты в watch-режиме |
| `npm run test:coverage` | Тесты с отчётом о покрытии |
| `npm run typecheck` | Проверить типы в `src/` и `tests/` без сборки |
...
- **Поиск не находит кириллицу.** `GameService.search()` приводит запрос к нижнему регистру и использует `contains` без `mode: 'insensitive'` (Prisma не поддерживает его для SQLite). Встроенный `LIKE` в SQLite сворачивает регистр только для ASCII, поэтому латиница ищется в любом регистре, а вот «Ведьмак» не находится ни по запросу «ведьмак», ни по «ВЕДЬМАК» — в русскоязычном каталоге это заметно. Зафиксировано тестом в `tests/db/gameService.test.ts`.
...
- **Четыре теста падают намеренно.** Сюита покрывает парсеры, сервисы и API (`npm test`), но баги из этого списка зафиксированы тестами на *корректное* поведение — они красные и служат списком задач: разбор `searchStore.elements` у Epic, нечисловая оригинальная цена при скидке 100% у Steam, кириллический поиск и падение `/api/games/search` при повторённом параметре `q`. Тесты с пометкой «БУДУЩИЙ БАГФИКС» в названии, наоборот, закрепляют текущее поведение и зелёные.
