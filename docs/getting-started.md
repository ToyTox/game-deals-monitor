...
> По умолчанию `RUN_ON_STARTUP` включён, и парсеры стартуют вместе с сервером — но в фоне: порт открывается сразу, API отвечает с первой секунды, а игры появляются по мере обхода площадок (первый прогон — несколько минут). `RUN_ON_STARTUP=false` нужен, только если стартовый прогон не нужен вовсе; расписание он не отключает.
...
| `VKPLAY_CONCURRENCY` | `3` | Сколько страниц каталога VK Play грузить одновременно; на 8 API начинает рвать соединения |
| `VKPLAY_RETRY_DELAY` | `1000` | Базовая пауза перед повтором упавшей страницы VK Play, мс; растёт с номером попытки (всего 3) |
